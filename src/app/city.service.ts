...
deg : any[] = [
  ...
  _addCity(user : User, city : City){
    return this.http.put<any>(`http://localhost:5002/user/ville/add`,{
      "_id" : user._id,
      "ville" : city.name,
      "pay" : city.country,
      "lon" : city.lon,
  ...
  _deleteFollowedCity(city : String){
    ...
    return this.http.delete<any>(`http://localhost:5002/user/ville/delete/${this.userService.user?._id["$oid"]}/${city}`);
  }
